...
用法：
  python scripts/run_eval_no_abstain.py --dataset datasets/crud_read_eval.csv --max-rows 50
"""
...
# 针对false_abstain优化的配置
os.environ["EVAL_PROMPT_STYLE"] = "crud_optimized_v4"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="使用针对false_abstain优化的提示词进行评测")
    parser.add_argument(
        "--dataset",
        type=Path,
    ...
    print(f"  - 提示词: crud_optimized_v4（针对false_abstain优化）")
    print(f"  - 核心原则：部分作答优于整体拒答")
    print(f"  - 实体匹配：宽松（即使人名/机构名不完全一致，只要内容相关就回答）")
    ...
    output = ROOT / "datasets" / f"eval_no_abstain_n{len(rows)}_{timestamp}"
    ...
    # 批量评测
    print(f"\n开始评测 {len(rows)} 条问题（针对false_abstain优化）...")

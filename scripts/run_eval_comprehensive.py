#!/usr/bin/env python3
"""
综合优化评测：结合 CRUD_RAG 最佳实践 + 增大召回 + 提示词优化
目标：将准确率提升到 55-60%，false_abstain 降到 15% 以下。

优化策略：
1. chunk_size: 128（CRUD_RAG 最佳实践）
2. top_k: 10（略微增大召回）
3. RERANK_RECALL_MULT: 4（平衡精度和召回）
4. 温度: 0.1（CRUD_RAG 最佳实践）
5. 提示词: crud_optimized（角色设定 + 结构化输出）

用法：
  python scripts/run_eval_comprehensive.py --dataset datasets/crud_read_eval.csv --max-rows 30
"""
...
os.environ["RETRIEVAL_VECTOR_TOP_K"] = "10"
os.environ["RETRIEVAL_KEYWORD_TOP_K"] = "10"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="综合优化评测")
    parser.add_argument(
        "--dataset",
        type=Path,
    ...
    parser.add_argument(
        "--vector-top-k",
        type=int,
        default=int(os.getenv("RETRIEVAL_VECTOR_TOP_K", "10")),
        help="向量检索 top-k",
    )
    parser.add_argument(
        "--keyword-top-k",
        type=int,
        default=int(os.getenv("RETRIEVAL_KEYWORD_TOP_K", "10")),
        help="关键词检索 top-k",
    )
    parser.add_argument(
        "--graph-max",
    ...
    print(f"  - vector_top_k: {args.vector_top_k}（平衡召回和精度）")
    print(f"  - keyword_top_k: {args.keyword_top_k}（平衡召回和精度）")
    print(f"  - graph_max: {args.graph_max}（平衡召回和精度）")
    print(f"  - RERANK_RECALL_MULT: 4（balance）")
    ...
    output = ROOT / "datasets" / f"eval_comprehensive_n{len(rows)}_{timestamp}"

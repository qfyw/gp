#!/usr/bin/env python3
"""
终极优化评测：超大数据召回 + 完整回答提示词
预期准确率：35% → 45-50%

优化策略：
1. 超大召回：top_k 从 8 增加到 15
2. 重排序上限增加到 20
3. 改进提示词：crud_optimized_v2（强调完整回答）
4. CRUD-RAG 最佳实践：chunk_size=128, 温度=0.1

用法：
  python scripts/run_eval_ultimate.py --dataset datasets/crud_read_eval.csv --max-rows 20
"""
...
os.environ["RETRIEVAL_VECTOR_TOP_K"] = "15"
os.environ["RETRIEVAL_KEYWORD_TOP_K"] = "15"
os.environ["RETRIEVAL_GRAPH_MAX"] = "12"
os.environ["RERANK_RECALL_MULT"] = "5"
os.environ["RRF_K"] = "60"
os.environ["RERANK_DOC_CAP"] = "20"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="终极优化评测：超大数据召回+完整回答")
    parser.add_argument(
        "--dataset",
        type=Path,
    ...
    parser.add_argument(
        "--vector-top-k",
        type=int,
        default=int(os.getenv("RETRIEVAL_VECTOR_TOP_K", "15")),
        help="向量检索 top-k",
    )
    parser.add_argument(
        "--keyword-top-k",
        type=int,
        default=int(os.getenv("RETRIEVAL_KEYWORD_TOP_K", "15")),
        help="关键词检索 top-k",
    )
    parser.add_argument(
        "--graph-max",
    ...
    print(f"  - vector_top_k: {args.vector_top_k}（超大数据召回）")
    print(f"  - keyword_top_k: {args.keyword_top_k}（超大数据召回）")
    print(f"  - graph_max: {args.graph_max}（从8增大到12）")
    print(f"  - RERANK_RECALL_MULT: 5（从4增大到5）")
    print(f"  - RERANK_DOC_CAP: 20（重排序上限）")
    print(f"  - RRF_K: 60（CRUD-RAG 默认）")
    print(f"  - temperature: {args.temperature}（CRUD-RAG 最佳实践）")
    print(f"  - 提示词: crud_optimized_v2（完整回答强调）")
    ...
    output = ROOT / "datasets" / f"eval_ultimate_n{len(rows)}_{timestamp}"

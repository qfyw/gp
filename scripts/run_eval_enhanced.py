#!/usr/bin/env python3
"""
使用 CRUD_RAG 最佳实践 + 增强版提示词（crud_optimized_v3）进行评测。
主要改进：
1. chunk_size: 128（CRUD-RAG 最佳实践）
2. chunk_overlap: 50（保持上下文连续性）
3. top_k: 12-15（增大召回）
4. 温度: 0.1（降低随机性）
5. 提示词优化：crud_optimized_v3（增强完整回答要求）
...
def main() -> int:
    parser = argparse.ArgumentParser(description="使用增强版配置进行评测")
    parser.add_argument(
        "--dataset",
        type=Path,
    ...
    print(f"  - vector_top_k: {args.vector_top_k}（增大召回）")
    print(f"  - keyword_top_k: {args.keyword_top_k}（增大召回）")
    print(f"  - graph_max: {args.graph_max}")
    print(f"  - RERANK_RECALL_MULT: 4（增大召回）")
    ...
    output = ROOT / "datasets" / f"eval_enhanced_n{len(rows)}_{timestamp}"

#!/usr/bin/env python3
"""
使用 CRUD_RAG 最佳实践进行评测。
主要改进：
1. chunk_size: 600→128（提高检索精度）
2. chunk_overlap: 0→50（保持上下文连续性）
3. top_k: 12→8（减少低质量结果）
4. 温度: 默认→0.1（降低随机性）
5. 提示词优化：借鉴 CRUD_RAG 的角色设定和结构化输出
...
def main() -> int:
    parser = argparse.ArgumentParser(description="使用 CRUD_RAG 最佳实践进行评测")
    parser.add_argument(
        "--dataset",
        type=Path,
    ...
    print(f"加载向量库与知识图谱…")
    print(f"CRUD_RAG 最佳实践配置：")
    print(f"  - chunk_size: 128（从600优化）")
    print(f"  - chunk_overlap: 50（从0优化）")
    print(f"  - vector_top_k: {args.vector_top_k}（从12优化）")
    print(f"  - keyword_top_k: {args.keyword_top_k}（从12优化）")
    print(f"  - graph_max: {args.graph_max}（从12优化）")
    print(f"  - RERANK_RECALL_MULT: 3（从5优化）")
    print(f"  - RRF_K: 60（CRUD_RAG 默认）")
    print(f"  - temperature: {args.temperature}（CRUD_RAG 默认）")
    ...
    output = ROOT / "datasets" / f"eval_crud_best_n{len(rows)}_{timestamp}"
    ...
    # 批量评测
    print(f"\n开始评测 {len(rows)} 条问题（CRUD_RAG 最佳实践）...")
